...
def podar_backups_locales(dejar=1):
    ...
    recientes — se llama ANTES de cualquier otra cosa (incluso antes de
    restaurar_si_hace_falta, que también escribe en disco) porque borrar no
    necesita espacio libre, a diferencia de escribir. Es el único recurso
    real cuando el volumen se llenó del todo y ni una INSERT normal puede
    ejecutarse (visto en Railway con un volumen de 500 MB): sin esto, la app
    no llega ni a arrancar para que la rotación normal de backups.py actúe.
